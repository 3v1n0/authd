...
// NewGroupManager creates a new GroupManager instance with the provided options.
...
func (g *GroupManager) SaveEntries(ctx context.Context, groups []types.GroupEntry) (err error) {
...
func (g *GroupManager) Update(ctx context.Context, username string, newGroups []string, oldGroups []string) (err error) {
	...
	return g.SaveEntries(ctx, allGroups)
}

func parseLocalGroups(groupFile string) (groups []types.GroupEntry, err error) {
	defer decorate.OnError(&err, "could not parse local group file %s", groupFile)

	log.Debugf(context.Background(), "Parsing local group file %s", groupFile)
