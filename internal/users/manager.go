...
func (m *Manager) UpdateUser(u types.UserInfo) (err error) {
	...
	checkEqualUserExists := func() (exists bool, err error) {
		...
		currentUser, err = m.getCurrentUserFromDB(u.Name)
		if err != nil {
			return false, err
		}
		if currentUser == nil {
		...
		if !compareNewUserWithUserFromDB(u, *currentUser) {
	...
	if currentUser != nil {
		...
		if preauthUID != 0 {
			...
			var cleanupUID func()
			u.UID, cleanupUID, err = m.idGenerator.GenerateUID(ctx, m)
			if err != nil {
				return err
			}
			defer cleanupUID()
...
func (m *Manager) getCurrentUserFromDB(name string) (currentUser *types.UserInfo, err error) {
	// Check if the user already exists in the database.
...
func compareNewUserWithUserFromDB(u types.UserInfo, dbUser types.UserInfo) bool {
	if len(dbUser.Groups) != len(u.Groups) {
		return false
	}
	...
	for idx, g := range u.Groups {
		oldGroupIdx := slices.IndexFunc(dbUser.Groups, func(dg types.GroupInfo) bool {
			if dg.UGID == "" {
		...
		u.Groups[idx].GID = dbUser.Groups[oldGroupIdx].GID
	}

	return dbUser.Equals(u)
}
