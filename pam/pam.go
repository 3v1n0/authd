...
var pamModuleHandler pam.ModuleHandler = &pamModule{} //nolint:golint,unused
...
func (h *pamModule) Authenticate(mt pam.ModuleTransaction, flags pam.Flags,
	...
	fmt.Println("GDM PROTOCOL SUPPORTED",
		gdm.IsPamExtensionSupported(gdm.PamExtensionPrivateString))
	...
	var status = newPamStatus(pam.ErrSystem, errors.New("unknown exit code"))
	...
	if status.Status() != pam.ErrIgnore {
		...
		if _, err := mt.StartStringConv(pam.ErrorMsg, status.Error()); err != nil {
			log.Errorf(context.TODO(), "PAM module got error: %v", err)
		}
