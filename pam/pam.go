...
func (h *pamModule) Authenticate(mTx pam.ModuleTransaction, flags pam.Flags,
	...
	fmt.Println("GDM PROTOCOL SUPPORTED",
		gdm.IsPamExtensionSupported(gdm.PamExtensionPrivateJSON))

	if gdm.IsPamExtensionSupported(gdm.PamExtensionPrivateJSON) {
		isGdm = true
	...
	case pamSuccess:
		if err := mTx.SetData(authenticationBrokerIDKey, exitMsg.brokerID); err != nil {
			return err
		}
		return nil
	case pamIgnore:
		// localBrokerID is only set on pamIgnore if the user has chosen local broker.
		if err := mTx.SetData(authenticationBrokerIDKey, exitMsg.localBrokerID); err != nil {
			return err
		}
		status = exitMsg
