...
func (m *authenticationModel) Update(msg tea.Msg) (authenticationModel, tea.Cmd) {
	...
	case isAuthenticatedResultReceived:
		...
		case responses.AuthRetry:
			...
			if err != nil {
				return *m, sendEvent(newPamStatus(pam.ErrSystem, err))
			}
			m.errorMsg = errorMsg
		...
		case responses.AuthDenied:
			...
			return *m, sendEvent(newPamStatus(pam.ErrAuth, errors.New(errMsg)))
...
func (m *authenticationModel) Compose(brokerID, sessionID string, layout *authd.UILayout) tea.Cmd {
	...
	case "qrcode":
		...
		if err != nil {
			return sendEvent(newPamStatus(pam.ErrSystem, err))
		}
	...
	default:
		return sendEvent(newPamStatus(pam.ErrSystem,
