...
func (m *authenticationModel) Update(msg tea.Msg) (authModel authenticationModel, command tea.Cmd) {
	...
	case isAuthenticatedRequested:
		...
		// message, until the previous request has been handled.
		...
		// At the point that we proceed with the actual authentication request,
		// there may still an authentication in progress, so send the request only
		// after we've completed the previous one.
		return *m, func() tea.Msg {
...
func (at *authTracker) reset() {
	at.cond.L.Lock()
	defer at.cond.L.Unlock()
	log.Infof(context.TODO(), "%p reset %v (%p)", at, at.inProgress, &at.inProgress)
	at.inProgress = false
	at.cond.Signal()
}
