...
setup_shared_module_data (pam_handle_t *pamh)
{
  ...
  if (pam_get_data (pamh, "go-exec-module-data", (const void **) &module_data) == PAM_SUCCESS)
    return module_data;

  module_data = g_new0 (ModuleData, 1);
  module_data->pamh = pamh;
  pam_set_data (pamh, "go-exec-module-data", module_data, on_exec_module_removed);
  ...
  if (is_debug_logging_enabled ())
    {
      ...
      g_debug ("Called method %s(%s)", method_name, args);
    }
  ...
  if (g_str_equal (method_name, "SetItem"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation,
        g_variant_new ("(i)", pam_set_item (module_data->pamh, item, value)));
    }
  else if (g_str_equal (method_name, "GetItem"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation,
        g_variant_new ("(is)", ret, (const char *) value ? value : ""));
    }
  else if (g_str_equal (method_name, "SetEnv"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation,
        g_variant_new ("(i)", pam_putenv (module_data->pamh, name_value)));
  ...
  else if (g_str_equal (method_name, "UnsetEnv"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation,
        g_variant_new ("(i)", pam_putenv (module_data->pamh, env)));
    }
  else if (g_str_equal (method_name, "GetEnv"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation,
        g_variant_new ("(is)", PAM_SUCCESS, value ? value : ""));
    }
  else if (g_str_equal (method_name, "GetEnvList"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation, g_variant_new ("(a{ss})",
                                             &dict_builder));
    }
  else if (g_str_equal (method_name, "SetData"))
    {
      ...
      ret = pam_set_data (module_data->pamh, key, variant, on_variant_data_removed);
  ...
  else if (g_str_equal (method_name, "UnsetData"))
    {
      ...
      g_dbus_method_invocation_return_value (invocation,
        g_variant_new ("(i)", pam_set_data (module_data->pamh, key, NULL, NULL)));

      const void *value;
      int ret = pam_get_data (module_data->pamh, key, &value);
      g_print("After unsetting, data for item %d: %p\n", ret, value);
    }
  else if (g_str_equal (method_name, "GetData"))
    {
      ...
      g_variant_get (parameters, "(&s)", &key);
      ret = pam_get_data (module_data->pamh, key, (const void **) &variant);
...
// static void
// call_done (GObject *source_object,
//                   GAsyncResult *res,
//                   void * user_data)
// {
//   g_autoptr(GError) error = NULL;
...
//   g_critical ("%s: can't call DBus: %s", "foo", error->message);
// }

// static void
// connection_ready (GObject *source_object,
//                   GAsyncResult *res,
//                   void * user_data)
// {
//   g_autoptr(GError) error = NULL;
...
//   c = g_dbus_connection_new_for_address_finish(res, &error);
...
//                           "com.ubuntu.authd.pam",
//                           "GetItem",
//                           g_variant_new ("(i)", PAM_USER),
//                           NULL,
//                           G_DBUS_CALL_FLAGS_NONE,
//                           -1, /* timeout_msec */
//                           NULL, /* GCancellable */
//                           call_done, module_data);
// }

// static gboolean
// on_connection_got (void * data)
// {
//   ModuleData  *module_data = data;

//   g_print("SERVER SETUP\n");

//   g_dbus_connection_new_for_address (g_dbus_server_get_client_address (module_data->server),
                   ...
                   G_GNUC_UNUSED void * user_data)
{
  ...
  // FIXME: Remove the connection when client goes away...
  if (module_data->connection)
    {
                   ...
                   GError     **error)
{
  ...
  // FIXME: add a mutex, ensuring that we can connect just once
  ...
  g_print ("DBus connection setup at %s\n", server_addr);
  ...
  // module_data->new_connection_id =
  //   g_signal_connect (server, "new-connection", G_CALLBACK (on_new_connection), module_data);
  g_dbus_server_start (server);

  module_data->server = g_steal_pointer (&server);
  g_print ("Connectable address: %s\n", g_dbus_server_get_client_address (module_data->server));

  // g_idle_add (on_connection_got, module_data);

  return TRUE;
  // return g_file_set_contents("/tmp/pam-module-address", g_dbus_server_get_client_address (module_data->server), -1, error);
}
                       ...
                       GError **error)
{
  ...
  if (out_args)
    *out_args = g_steal_pointer (&args_strv);
               ...
               const char  **argv)
{
  ...
  if (!handle_module_options(argc, argv, &exec_args, &env_variables, &error))
    {
  ...
  argc = g_strv_length (exec_args);
  argv = (const char **) exec_args;
  ...
  if G_UNLIKELY (module_data->child_pid)
    {
      ...
      g_debug ("Another client with PID %"G_PID_FORMAT
               " is still running, let's wait for it", module_data->child_pid);
  ...
  args = g_ptr_array_new_full (argc + 9, g_free);
  g_ptr_array_add (args, g_strdup (exe));
  g_ptr_array_add (args, g_strdup ("-flags"));
  g_ptr_array_add (args, g_strdup_printf("%d", flags));
  g_ptr_array_add (args, g_strdup ("-server-address"));
  g_ptr_array_add (args, g_strdup (g_dbus_server_get_client_address(module_data->server)));
  ...
  // if (!g_option_context_parse_strv (options_context, &args_strv, &error))
  //   {
  //     log_error (pamh, action, "impossible to parse arguments: %s", error->message);
  //     return PAM_SYSTEM_ERR;
  //   }
  ...
  // if (!isatty (STDIN_FILENO))
  //   {
  //     log_error (p action,amh, "not an interactive terminal");
  //     return PAM_SYSTEM_ERR;
  //   }
  ...
  //                     (GStrv) envp->pdata,
  //                     G_SPAWN_DO_NOT_REAP_CHILD |
  //                     interactive_mode ? G_SPAWN_CHILD_INHERITS_STDIN : 0,
  //                     NULL, NULL, /* Child setup */
  //                     &child_pid,
  //                     &error))
    {
