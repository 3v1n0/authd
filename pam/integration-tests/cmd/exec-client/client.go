...
func handleArg(mTx pam.ModuleTransaction, arg string) error {
	...
	//   {"action": <"$action">, "args": <[$input...]>, "expects": <[$expected...]>}
	// Where:
	...
	// For example:
	//   {"action": <"SetItem">, "args": <[<2>, <"user">]>}
	// or:
	//   {"action": <"SetItem">, "args": <[<-1>, <"foo">]>, "ret": <[<29>]>}
	...
	action, err := getVariantMapItem[string](parsedArg, "action")
	if err != nil {
	...
	expectedRet, err := getVariantMapItem[[]dbus.Variant](parsedArg, "expects")
	if err != nil {
...
func checkReturnedValues(action string, method reflect.Value, wantArgs []dbus.Variant, retValues []reflect.Value) error {
	...
	for idx, wantArg := range wantArgs {
		...
		return fmt.Errorf("values do not match: expected '%#v', got '%#v': %w",
			wantValue, retValue, pam_test.ErrReturnMismatch)
	}
