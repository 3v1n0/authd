...
func TestExecModule(t *testing.T) {
	...
	gdmConvTests := map[string]struct {
		...
		"Error if starting conversation without GDM support in module": {
			jsonRequest:           []byte("This is not really happening"),
			wantError:             pam.ErrConv,
			disableGdmInModule:    true,
			convShouldNotBeCalled: true,
		},
	}
	for name, tc := range gdmConvTests {
		t.Run("GdmConv "+name, func(t *testing.T) {
			if !tc.disableGdmInModule {
				t.Parallel()
			} else {
			...
			if tc.disableGdmInModule {
				moduleArgs = append(moduleArgs, "-enable-gdm=true")
			}
