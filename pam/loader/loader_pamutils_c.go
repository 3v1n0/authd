...
var GDM_PAM_EXTENSION_PRIVATE_STRING string = C.GDM_PAM_EXTENSION_PRIVATE_STRING
...
func formatAuthDJSonProtoReply(reply []byte) []byte {
	response := C.GdmPamExtensionStringProtocol{}
	cProtoName := C.CString(gdm.StringProtoName)
	defer C.free(unsafe.Pointer(cProtoName))

	C.gdm_private_string_response_init(&response, cProtoName,
		C.uint(gdm.StringProtoVersion))

	// These are stolen as will be owned by the module
	response.value = C.CString(string(reply))
	fmt.Println("ALLOCATED LOADER", unsafe.Pointer(response.value))
...
func handleBinaryProtocol(message *C.struct_pam_message) (int, []byte) {
	...
	if C.GoString(proto) == gdm.StringProtoName && uint(version) == gdm.StringProtoVersion {
