...
func initPamHandle(serviceName string, user string, servicePath string) (pamHandle, error) {
	var cError *C.char
	if pamh := C.init_handle(C.CString(serviceName),
		C.CString(user), C.CString(servicePath), &cError); pamh != nil {
		return pamh, nil
	}
...
func formatAuthDJSonProtoReply(reply []byte) []byte {
	...
	return C.GoBytes(unsafe.Pointer(cPamReply), C.int(cPamReplySize))
}
...
func handleBinaryProtocol(message *C.struct_pam_message) (int, []byte) {
	...
	if C.GoString(proto) == gdm.ProtoName && int(version) == gdm.ProtoVersion {
		retValue, err := handleAuthDJsonProto([]byte(C.GoString(value)))
		if err != nil {
			log.Error(context.TODO(), err)
