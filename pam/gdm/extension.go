...
const (
	// PamExtensionPrivateJSON is the gdm PAM extension for passing string values.
	PamExtensionPrivateJSON = C.GDM_PAM_EXTENSION_PRIVATE_JSON
...
func init() {
	...
	if unsafe.Sizeof(msg) != C.GDM_PAM_EXTENSION_PRIVATE_JSON_SIZE {
		panic(fmt.Sprintf("unexpected request size: %v vs %v", unsafe.Sizeof(msg),
			C.GDM_PAM_EXTENSION_PRIVATE_JSON_SIZE))
	}

	if uintptr(len(JSONProtoName)) >= unsafe.Sizeof(msg.protocol_name) {
		panic(fmt.Sprintf("Protocol version '%s' exceeds maximum size of %d",
			JSONProtoName, unsafe.Sizeof(msg.protocol_name)))
	}
}
...
func (msg *jsonProtoMessage) init(protoName string, protoVersion uint, jsonValue []byte) {
	...
	C.gdm_private_json_request_init(msg, cProto, C.uint(protoVersion), cJSON)
}
...
func decodeJSONProtoMessage(response pam.BinaryPointer) ([]byte, error) {
	...
	if reply.protoName() != JSONProtoName ||
		...
		return nil, fmt.Errorf(
			"protocol name or version mismatch: got %s v%d, expected %s v%d",
