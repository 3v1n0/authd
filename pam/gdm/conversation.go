...
func (d *Data) SendParsed(pamMTx pam.ModuleTransaction) (*Data, error) {
	...
	// We protect do an explicit check to avoid any unneeded string allocation.
	if log.GetLevel() == log.DebugLevel {
		log.Debugf(context.TODO(), "Sending to GDM: %v", string(bytes))
	}
	...
	if log.GetLevel() == log.DebugLevel {
		log.Debugf(context.TODO(), "Sending to GDM: %v", string(jsonValue))
	}
