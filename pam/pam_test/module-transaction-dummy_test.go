...
func TestSetGetItem(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"set invalid item": {
			...
			expectedSetError: pam.ErrBadItem,
		},

		"get invalid item": {
			item:             pam.Item(0),
			expectedGetError: pam.ErrBadItem,
...
func TestSetPutEnv(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"put invalid env": {
			...
			expectedPutError: pam.ErrBadItem,
		},
...
func TestSetGetData(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"get missing": {
			skipSet:          true,
			key:              "not set",
			expectedGetError: pam.ErrNoModuleData,
		},
...
func TestStartBinaryPointerConv(t *testing.T) {
	...
	tests := map[string]struct {
		request     []byte
		convError   error
		convHandler *pam.BinaryPointerConversationHandler
		...
		"no conv set fails": {
			convHandler:   ptrValue(pam.BinaryPointerConversationHandler(nil)),
			expectedError: pam.ErrConv,
		},
	}
	...
	for name, tc := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			tx := NewModuleTransactionDummy(func() pam.BinaryPointerConversationHandler {
				...
				return pam.BinaryPointerConversationFunc(
					func(ptr pam.BinaryPointer) (pam.BinaryPointer, error) {
						...
						require.Equal(t, tc.request, bytes)
						return allocateCBytes(tc.expected), tc.convError
					})
			}())
			...
			bytes, err := response.Decode(func(ptr pam.BinaryPointer) ([]byte, error) {
				return cBytesToBytes(ptr, len(tc.expected)), nil
			})
			require.NoError(t, err)
			require.Equal(t, tc.expected, bytes)
