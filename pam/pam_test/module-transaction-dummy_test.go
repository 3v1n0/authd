...
func TestSetGetItem(t *testing.T) {
	...
	for name, tc := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.Truef(t, tc.value != nil || tc.expectedValue != nil,
				"Setup: test case is missing data, no test will run")
...
func (c *multiConvHandler) RespondPAMBinary(ptr pam.BinaryPointer) ([]byte, error) {
	...
	binReq, ok := expectedReq.(*pam.BinaryConvRequest)
	require.True(c.t, ok)
...
func TestStartConvMulti(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"binary requests": {
			requests: []pam.ConvRequest{
				pam.NewBinaryConvRequest(nil, nil),
				pam.NewBinaryConvRequest(pam.BinaryPointer(&[]byte{}), nil),
				pam.NewBinaryConvRequest(pam.BinaryPointer(&[]byte{0xFF, 0x00, 0xBA, 0xAB}), nil),
				pam.NewBinaryConvRequest(pam.BinaryPointer(&[]byte{0x55}), nil),
			},
			expectedResponses: []pam.ConvResponse{
